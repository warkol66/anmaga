...
class UserPeer extends BaseUserPeer {
  ...
  * @return el objeto del usuario si se creo el usuario correctamente, false sino
	*/
  ...
  */
  ...
  /**
  * Relaciona un usuario a un Supplier.
  *
  * @param int $userId Id del usuario
  * @param int $supplierId Id del grupo de usuarios
  * @return boolean true si se agrego correctamente, false sino
  * TODO: Revisar bug de porque propel no actualiza al estado correcto.
  */
	function linkUserWithSupplier($userId,$supplierId) {

		$crit = new Criteria();
		$crit->add(SupplierUserPeer::USERID, $userId);
		$results = SupplierUserPeer::doSelect($crit);

		if (empty($results)) {
			//no existia la relacion
			$supplierUser = new SupplierUser();
			$supplierUser->setUserId($userId);
			
		}
		else {
			//existia, tenemos que editarla
			$supplierUser = $results[0];
  ...
  /**
  * Agrega un usuario a un grupo de suppliers.
  *
  * @param int $user Id del usuario
  * @return boolean true si se agrego correctamente, false sino
  */

	function addUserToSupplierGroup($user) {
		
		//se asigna al grupo de id tres, que es el de suppliers
		return $this->addUserToGroup($user,'3');
