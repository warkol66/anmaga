...
abstract class BaseUser extends BaseObject  implements Persistent {
	...
	/**
	 * Collection to store aggregation of collRequests.
	 * @var        array
	 */
	protected $collRequests;

	/**
	 * The criteria used to select the current contents of collRequests.
	 * @var        Criteria
	 */
	protected $lastRequestCriteria = null;
	...
	/**
	 * Temporary storage of collRequests to save a possible db hit in
	 * the event objects are add to the collection, but the
	 * complete collection is never requested.
	 * @return     void
	 */
	 ...
	 * @throws     PropelException
	 */
	public function getRequests($criteria = null, $con = null)
	{
		// include the Peer class
		...
		if ($this->collRequests === null) {
			if ($this->isNew()) {
			   $this->collRequests = array();
				...
				$criteria->add(RequestPeer::USERID, $this->getId());
	...
	/**
	 * Returns the number of related Requests.
	 *
	 * @param      Criteria $criteria
	 * @param      boolean $distinct
	 * @param      Connection $con
	 * @throws     PropelException
	 */
	public function countRequests($criteria = null, $distinct = false, $con = null)
	{
		// include the Peer class
		include_once 'anmaga/om/BaseRequestPeer.php';
		if ($criteria === null) {
		...
		return RequestPeer::doCount($criteria, $distinct, $con);
	}
		...
		if ($this->collSupplierUsers === null) {
			if ($this->isNew()) {
			   $this->collSupplierUsers = array();
				...
				$criteria->add(SupplierUserPeer::USERID, $this->getId());
	 ...
	 * actually need in User.
	 */
	public function getSupplierUsersJoinSupplier($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'anmaga/om/BaseSupplierUserPeer.php';
		if ($criteria === null) {
		...
		if ($this->collSupplierUsers === null) {
			if ($this->isNew()) {
				...
				$this->collSupplierUsers = SupplierUserPeer::doSelectJoinSupplier($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
