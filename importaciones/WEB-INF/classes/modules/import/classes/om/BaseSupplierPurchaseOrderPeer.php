...
abstract class BaseSupplierPurchaseOrderPeer {
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id', 'createdat', 'supplierid', 'status', 'timestampstatus', 'supplierquotationid', ),
		BasePeer::TYPE_COLNAME => array (self::ID, self::CREATEDAT, self::SUPPLIERID, self::STATUS, self::TIMESTAMPSTATUS, self::SUPPLIERQUOTATIONID, ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_STUDLYPHPNAME => array ('id' => 0, 'createdat' => 1, 'supplierid' => 2, 'status' => 3, 'timestampstatus' => 4, 'supplierquotationid' => 5, ),
		BasePeer::TYPE_COLNAME => array (self::ID => 0, self::CREATEDAT => 1, self::SUPPLIERID => 2, self::STATUS => 3, self::TIMESTAMPSTATUS => 4, self::SUPPLIERQUOTATIONID => 5, ),
	...
	public static function getMapBuilder()
	{
		if (self::$mapBuilder === null) {
			self::$mapBuilder = new SupplierPurchaseOrderMapBuilder();
		}
	...
	public static function doSelectJoinSupplierQuotation(Criteria $c, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (SupplierPurchaseOrder) to $obj2 (SupplierQuotation)
	...
	public static function doSelectJoinAllExceptSupplierQuotation(Criteria $c, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if (null !== ($obj1 = SupplierPurchaseOrderPeer::getInstanceFromPool($key1))) {
				...
				// Add the $obj1 (SupplierPurchaseOrder) to the collection in $obj2 (Supplier)
				$obj2->addSupplierPurchaseOrder($obj1);
...
//
// NOTE: This static code cannot call methods on the SupplierPurchaseOrderPeer class, because it is not defined yet.
// If you need to use overridden methods, you can add this code to the bottom of the SupplierPurchaseOrderPeer class:
//
// Propel::getDatabaseMap(SupplierPurchaseOrderPeer::DATABASE_NAME)->addTableBuilder(SupplierPurchaseOrderPeer::TABLE_NAME, SupplierPurchaseOrderPeer::getMapBuilder());
//
// Doing so will effectively overwrite the registration below.

Propel::getDatabaseMap(BaseSupplierPurchaseOrderPeer::DATABASE_NAME)->addTableBuilder(BaseSupplierPurchaseOrderPeer::TABLE_NAME, BaseSupplierPurchaseOrderPeer::getMapBuilder());
