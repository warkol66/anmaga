...
abstract class BaseOrderTemplateItem extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'OrderTemplateItemPeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 5; // 5 = OrderTemplateItemPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = OrderTemplateItemPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				OrderTemplateItemPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	protected function doSave(PropelPDO $con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->aOrderTemplate !== null) {
				if ($this->aOrderTemplate->isModified() || $this->aOrderTemplate->isNew()) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(OrderTemplateItemPeer::ID) ) {
	...
	{
		$pos = OrderTemplateItemPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['OrderTemplateItem'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['OrderTemplateItem'][$this->getPrimaryKey()] = true;
		$keys = OrderTemplateItemPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
		...
		if ($includeForeignObjects) {
			if (null !== $this->aOrderTemplate) {
				$result['OrderTemplate'] = $this->aOrderTemplate->toArray($keyType, $includeLazyLoadColumns,  $alreadyDumpedObjects, true);
			}
	...
	{
		$copyObj->setOrdertemplateid($this->getOrdertemplateid());
	 ...
	 * @throws     PropelException
	 */
	public function setOrderTemplate(OrderTemplate $v = null)
	{
		if ($v === null) {
		...
		// If this object has already been added to the OrderTemplate object, it will not be re-added.
		if ($v !== null) {
			$v->addOrderTemplateItem($this);
		}
	 ...
	 * @throws     PropelException
	 */
	public function getOrderTemplate(PropelPDO $con = null)
	{
		if ($this->aOrderTemplate === null && ($this->ordertemplateid !== null)) {
			$this->aOrderTemplate = OrderTemplateQuery::create()->findPk($this->ordertemplateid, $con);
			/* The following can be used additionally to
				guarantee the related object contains a reference
				to this object.  This level of coupling may, however, be
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aOrderTemplate->addOrderTemplateItems($this);
			 */
		}
	...
	public function getProduct(PropelPDO $con = null)
	{
		if ($this->aProduct === null && (($this->productcode !== "" && $this->productcode !== null))) {
			...
			/* The following can be used additionally to
				...
				undesirable since it could result in an only partially populated collection
				in the referenced object.
				$this->aProduct->addOrderTemplateItems($this);
			 */
		}
	...
	{
		return (string) $this->exportTo(OrderTemplateItemPeer::DEFAULT_STRING_FORMAT);
	}
