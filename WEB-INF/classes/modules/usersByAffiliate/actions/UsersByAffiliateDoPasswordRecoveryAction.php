...
class UsersByAffiliateDoPasswordRecoveryAction extends BaseAction {


	// ----- Constructor ---------------------------------------------------- //

	function UsersByAffiliateDoPasswordRecoveryAction() {
		;
	}
	...
	*/
    ...
    BaseAction::execute($mapping, $form, $request, $response);
		...
		if ( !empty($_POST["username"]) && !empty($_POST["mailAddress"]) ) {
			$userAndPassword = UserByAffiliatePeer::generatePassword($_POST["username"],$_POST["mailAddress"]);
			if ( !empty($userAndPassword) ) {
        $smarty->assign("user",$userAndPassword[0]);
        $smarty->assign("password",$userAndPassword[1]);
        $body = $smarty->fetch("UsersByAffiliatePasswordRecoveryMail.tpl");

				$userInfo = $userAndPassword[0]->getUsersByAffiliateUserInfo();
