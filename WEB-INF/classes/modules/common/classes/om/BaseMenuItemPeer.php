...
/**
 * Base static class for performing query and update operations on the 'common_menuItem' table.
 *
...
abstract class BaseMenuItemPeer {
	...
	const CLASS_DEFAULT = 'common.classes.MenuItem';
	...
	const ACTION = 'common_menuItem.ACTION';

	/** the column name for the URL field */
	const URL = 'common_menuItem.URL';

	/** the column name for the ORDER field */
	const ORDER = 'common_menuItem.ORDER';

	/** the column name for the PARENTID field */
	const PARENTID = 'common_menuItem.PARENTID';
	...
	private static $fieldNames = array (
		...
		BasePeer::TYPE_RAW_COLNAME => array ('ID', 'ACTION', 'URL', 'ORDER', 'PARENTID', 'NEWWINDOW', ),
		BasePeer::TYPE_FIELDNAME => array ('id', 'action', 'url', 'order', 'parentId', 'newWindow', ),
	...
	private static $fieldKeys = array (
		...
		BasePeer::TYPE_RAW_COLNAME => array ('ID' => 0, 'ACTION' => 1, 'URL' => 2, 'ORDER' => 3, 'PARENTID' => 4, 'NEWWINDOW' => 5, ),
		BasePeer::TYPE_FIELDNAME => array ('id' => 0, 'action' => 1, 'url' => 2, 'order' => 3, 'parentId' => 4, 'newWindow' => 5, ),
		BasePeer::TYPE_NUM => array (0, 1, 2, 3, 4, 5, )
	);
	...
	public static function addSelectColumns(Criteria $criteria, $alias = null)
	{
		if (null === $alias) {
			...
			$criteria->addSelectColumn(MenuItemPeer::ACTION);
			$criteria->addSelectColumn(MenuItemPeer::URL);
			$criteria->addSelectColumn(MenuItemPeer::ORDER);
			$criteria->addSelectColumn(MenuItemPeer::PARENTID);
			$criteria->addSelectColumn(MenuItemPeer::NEWWINDOW);
		} else {
			...
			$criteria->addSelectColumn($alias . '.NEWWINDOW');
		}
	}
	...
	/**
	 * Method to invalidate the instance pool of all tables related to common_menuItem
	 * by a foreign key with ON DELETE CASCADE
	 */
	public static function clearRelatedInstancePool()
	{
		// Invalidate objects in MenuItemInfoPeer instance pool, 
	...
	{
		$key = MenuItemPeer::getPrimaryKeyHashFromRow($row, $startcol);
		if (null !== ($obj = MenuItemPeer::getInstanceFromPool($key))) {
			// We no longer rehydrate the object, since this can cause data loss.
			// See http://www.propelorm.org/ticket/509
	...
	public static function doUpdate($values, PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$value = $criteria->remove(MenuItemPeer::ID);
			if ($value) {
				$selectCriteria->add(MenuItemPeer::ID, $value, $comparison);
			} else {
				$selectCriteria->setPrimaryTableName(MenuItemPeer::TABLE_NAME);
			}
	...
	/**
	 * Method to DELETE all rows from the common_menuItem table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += MenuItemPeer::doOnDeleteCascade(new Criteria(MenuItemPeer::DATABASE_NAME), $con);
			$affectedRows += BasePeer::doDeleteAll(MenuItemPeer::TABLE_NAME, $con, MenuItemPeer::DATABASE_NAME);
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		try {
			...
			$c = clone $criteria;
			$affectedRows += MenuItemPeer::doOnDeleteCascade($c, $con);
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		$objects = MenuItemPeer::doSelect($criteria, $con);
		foreach ($objects as $obj) {


			// delete related MenuItemInfo objects
			$criteria = new Criteria(MenuItemInfoPeer::DATABASE_NAME);
			
			$criteria->add(MenuItemInfoPeer::MENUITEMID, $obj->getId());
			$affectedRows += MenuItemInfoPeer::doDelete($criteria, $con);
		}
		return $affectedRows;
	}
