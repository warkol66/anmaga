...
/**
 * Base static class for performing query and update operations on the 'common_alertSubscription' table.
 *
 * Suscripciones de alerta
 *
 * @package    propel.generator.common.classes.om
 */
abstract class BaseAlertSubscriptionPeer {

	/** the default database name for this class */
	const DATABASE_NAME = 'application';

	/** the table name for this class */
	const TABLE_NAME = 'common_alertSubscription';

	/** the related Propel class for this table */
	const OM_CLASS = 'AlertSubscription';

	/** A class that can be returned by this peer. */
	const CLASS_DEFAULT = 'common.classes.AlertSubscription';
	...
	const NAME = 'common_alertSubscription.NAME';
	...
	const ENTITYNAMEFIELDUNIQUENAME = 'common_alertSubscription.ENTITYNAMEFIELDUNIQUENAME';

	/** the column name for the EXTRARECIPIENTS field */
	const EXTRARECIPIENTS = 'common_alertSubscription.EXTRARECIPIENTS';

	/**
	 * An identiy map to hold any loaded instances of AlertSubscription objects.
	 * This must be public so that other peer classes can access this when hydrating from JOIN
	 * queries.
	 * @var        array AlertSubscription[]
	 */
	 ...
	 * @param      string $column The column name for current table. (i.e. AlertSubscriptionPeer::COLUMN_NAME).
	 * @return     string
	 */
	public static function alias($alias, $column)
	{
		return str_replace(AlertSubscriptionPeer::TABLE_NAME.'.', $alias.'.', $column);
	}
	...
	public static function doSelectOne(Criteria $criteria, PropelPDO $con = null)
	{
		...
		$critcopy->setLimit(1);
		$objects = AlertSubscriptionPeer::doSelect($critcopy, $con);
		if ($objects) {
			return $objects[0];
		}
		return null;
	...
	{
		return AlertSubscriptionPeer::populateObjects(AlertSubscriptionPeer::doSelectStmt($criteria, $con));
	}
	/**
	 ...
	 *
	 * @param      AlertSubscription $value A AlertSubscription object.
	 * @param      string $key (optional) key to use for instance map (for performance boost if key was already calculated externally).
	 */
	public static function addInstanceToPool(AlertSubscription $obj, $key = null)
	{
	 ...
	 *
	 * @param      mixed $value A AlertSubscription object or a primary key value.
	 */
	public static function removeInstanceFromPool($value)
	{
		if (Propel::isInstancePoolingEnabled() && $value !== null) {
			if (is_object($value) && $value instanceof AlertSubscription) {
				$key = (string) $value->getId();
			} elseif (is_scalar($value)) {
				// assume we've been passed a primary key
				$key = (string) $value;
			} else {
				$e = new PropelException("Invalid value passed to removeInstanceFromPool().  Expected primary key or AlertSubscription object; got " . (is_object($value) ? get_class($value) . ' object.' : var_export($value,true)));
				throw $e;
			}
	 ...
	 * @return     AlertSubscription Found object or NULL if 1) no instance exists for specified key or 2) instance pooling has been disabled.
	...
	/**
	 * Method to invalidate the instance pool of all tables related to common_alertSubscription
	...
	public static function populateObjects(PDOStatement $stmt)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			$key = AlertSubscriptionPeer::getPrimaryKeyHashFromRow($row, 0);
			if (null !== ($obj = AlertSubscriptionPeer::getInstanceFromPool($key))) {
				...
				$obj->hydrate($row);
				$results[] = $obj;
				AlertSubscriptionPeer::addInstanceToPool($obj, $key);
			} // if key exists
		}
		$stmt->closeCursor();
		return $results;
	 ...
	 * @throws     PropelException Any exceptions caught during processing will be
	 *		 rethrown wrapped into a PropelException.
	 * @return     array (AlertSubscription object, last column rank)
	 */
	public static function populateObject($row, $startcol = 0)
	{
		$key = AlertSubscriptionPeer::getPrimaryKeyHashFromRow($row, $startcol);
		if (null !== ($obj = AlertSubscriptionPeer::getInstanceFromPool($key))) {
			// We no longer rehydrate the object, since this can cause data loss.
			// See http://www.propelorm.org/ticket/509
	...
	/**
	 * Selects a collection of AlertSubscription objects pre-filled with their ModuleEntity objects.
	 * @param      Criteria  $criteria
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	...
	public static function doSelectJoinModuleEntity(Criteria $criteria, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		...
		while ($row = $stmt->fetch(PDO::FETCH_NUM)) {
			...
			if ($key2 !== null) {
				...
				// Add the $obj1 (AlertSubscription) to $obj2 (ModuleEntity)
	...
	/**
	 * Selects a collection of AlertSubscription objects pre-filled with all related objects.
	 *
	 * @param      Criteria  $criteria
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	...
	/**
	 * Selects a collection of AlertSubscription objects pre-filled with all related objects except ModuleEntity.
	 *
	 * @param      Criteria  $criteria
	 * @param      PropelPDO $con
	 * @param      String    $join_behavior the type of joins to use, defaults to Criteria::LEFT_JOIN
	...
	{
	  $dbMap = Propel::getDatabaseMap(BaseAlertSubscriptionPeer::DATABASE_NAME);
	  if (!$dbMap->hasTable(BaseAlertSubscriptionPeer::TABLE_NAME))
	  {
	    $dbMap->addTableObject(new AlertSubscriptionTableMap());
	  }
	}
	...
	{
		return $withPrefix ? AlertSubscriptionPeer::CLASS_DEFAULT : AlertSubscriptionPeer::OM_CLASS;
	}

	/**
	 * Method perform an INSERT on the database, given a AlertSubscription or Criteria object.
	 *
	 * @param      mixed $values Criteria or AlertSubscription object containing data that is used to create the INSERT statement.
	...
	public static function doInsert($values, PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$criteria = $values->buildCriteria(); // build Criteria from AlertSubscription object
		}

		if ($criteria->containsKey(AlertSubscriptionPeer::ID) && $criteria->keyContainsValue(AlertSubscriptionPeer::ID) ) {
	...
	/**
	 * Method perform an UPDATE on the database, given a AlertSubscription or Criteria object.
	 *
	 * @param      mixed $values Criteria or AlertSubscription object containing data that is used to create the UPDATE statement.
	...
	public static function doUpdate($values, PropelPDO $con = null)
	{
		...
		if ($values instanceof Criteria) {
			...
			$comparison = $criteria->getComparison(AlertSubscriptionPeer::ID);
			$value = $criteria->remove(AlertSubscriptionPeer::ID);
			if ($value) {
				$selectCriteria->add(AlertSubscriptionPeer::ID, $value, $comparison);
			} else {
				$selectCriteria->setPrimaryTableName(AlertSubscriptionPeer::TABLE_NAME);
			}

		} else { // $values is AlertSubscription object
	...
	/**
	 * Method to DELETE all rows from the common_alertSubscription table.
	 *
	 * @return     int The number of affected rows (if supported by underlying database driver).
	 */
	public static function doDeleteAll($con = null)
	{
		...
		try {
			...
			$con->beginTransaction();
			$affectedRows += AlertSubscriptionPeer::doOnDeleteCascade(new Criteria(AlertSubscriptionPeer::DATABASE_NAME), $con);
			$affectedRows += BasePeer::doDeleteAll(AlertSubscriptionPeer::TABLE_NAME, $con, AlertSubscriptionPeer::DATABASE_NAME);
			// Because this db requires some delete cascade/set null emulation, we have to
			// clear the cached instance *after* the emulation has happened (since
			// instances get re-added by the select statement contained therein).
			AlertSubscriptionPeer::clearInstancePool();
			AlertSubscriptionPeer::clearRelatedInstancePool();
			$con->commit();
			return $affectedRows;
		} catch (PropelException $e) {
			$con->rollBack();
	...
	/**
	 * Method perform a DELETE on the database, given a AlertSubscription or Criteria object OR a primary key value.
	 *
	 * @param      mixed $values Criteria or AlertSubscription object or primary key or array of primary keys
	 ...
	 */
	 public static function doDelete($values, PropelPDO $con = null)
	 {
		...
		} elseif ($values instanceof AlertSubscription) { // it's a model object
			// create criteria based on pk values
			$criteria = $values->buildPkeyCriteria();
		} else { // it's a primary key, or an array of pks
			$criteria = new Criteria(self::DATABASE_NAME);
			$criteria->add(AlertSubscriptionPeer::ID, (array) $values, Criteria::IN);
		}
		...
		try {
			...
			$c = clone $criteria;
			$affectedRows += AlertSubscriptionPeer::doOnDeleteCascade($c, $con);
			...
			if ($values instanceof Criteria) {
				...
				AlertSubscriptionPeer::removeInstanceFromPool($values);
			} else { // it's a primary key, or an array of pks
				foreach ((array) $values as $singleval) {
					AlertSubscriptionPeer::removeInstanceFromPool($singleval);
				}
			}
	...
	protected static function doOnDeleteCascade(Criteria $criteria, PropelPDO $con)
	{
		...
		$objects = AlertSubscriptionPeer::doSelect($criteria, $con);
		foreach ($objects as $obj) {
			...
			$criteria->add(AlertSubscriptionUserPeer::ALERTSUBSCRIPTIONID, $obj->getId());
	...
	/**
	 * Validates all modified columns of given AlertSubscription object.
	 * If parameter $columns is either a single column name or an array of column names
	 * than only those columns are validated.
	 *
	 * NOTICE: This does not apply to primary or foreign keys for now.
	 *
	 * @param      AlertSubscription $obj The object to validate.
	 * @param      mixed $cols Column name or array of column names.
	 *
	 * @return     mixed TRUE if all columns are valid or the error message of the first invalid column.
	 */
	public static function doValidate(AlertSubscription $obj, $cols = null)
	{
		$columns = array();

		if ($cols) {
			$dbMap = Propel::getDatabaseMap(AlertSubscriptionPeer::DATABASE_NAME);
			$tableMap = $dbMap->getTable(AlertSubscriptionPeer::TABLE_NAME);
		...
		return BasePeer::doValidate(AlertSubscriptionPeer::DATABASE_NAME, AlertSubscriptionPeer::TABLE_NAME, $columns);
	}
	...
	public static function retrieveByPK($pk, PropelPDO $con = null)
	{
		...
		if (null !== ($obj = AlertSubscriptionPeer::getInstanceFromPool((string) $pk))) {
			return $obj;
		}
		...
		$criteria->add(AlertSubscriptionPeer::ID, $pk);

		$v = AlertSubscriptionPeer::doSelect($criteria, $con);
	...
	public static function retrieveByPKs($pks, PropelPDO $con = null)
	{
		...
		if (empty($pks)) {
			...
			$criteria = new Criteria(AlertSubscriptionPeer::DATABASE_NAME);
			$criteria->add(AlertSubscriptionPeer::ID, $pks, Criteria::IN);
			$objs = AlertSubscriptionPeer::doSelect($criteria, $con);
		}
		return $objs;
	}

} // BaseAlertSubscriptionPeer

// This is the static code needed to register the TableMap for this table with the main Propel class.
//
BaseAlertSubscriptionPeer::buildTableMap();
