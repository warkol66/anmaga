...
abstract class BaseUnit extends BaseObject  implements Persistent
{
	...
	/**
	 * The value for the unitquantity field.
	 * @var        int
	 */
	...
	/**
	 * Get the [unitquantity] column value.
	 * Cantidad de unidades que posee la unidad
	 * @return     int
	 */
	...
	/**
	 * Set the value of [unitquantity] column.
	 * Cantidad de unidades que posee la unidad
	 * @param      int $v new value
	 * @return     Unit The current object (for fluent API support)
	 */
	public function setUnitquantity($v)
	{
		if ($v !== null) {
			$v = (int) $v;
		}

		if ($this->unitquantity !== $v) {
			$this->unitquantity = $v;
			$this->modifiedColumns[] = UnitPeer::UNITQUANTITY;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->unitquantity = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = UnitPeer::NUM_HYDRATE_COLUMNS.
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(UnitPeer::ID) ) {
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[2], $arr)) $this->setUnitquantity($arr[$keys[2]]);
	}
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(UnitPeer::UNITQUANTITY)) $criteria->add(UnitPeer::UNITQUANTITY, $this->unitquantity);
	...
	public function copyInto($copyObj, $deepCopy = false, $makeNew = true)
	{
		...
		$copyObj->setUnitquantity($this->getUnitquantity());
	 ...
	 * @return     PropelCollection|array Product[] List of Product objects
	 */
	public function getProductsJoinMeasureUnit($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = ProductQuery::create(null, $criteria);
		$query->joinWith('MeasureUnit', $join_behavior);
	...
	{
		return (string) $this->exportTo(UnitPeer::DEFAULT_STRING_FORMAT);
	}
