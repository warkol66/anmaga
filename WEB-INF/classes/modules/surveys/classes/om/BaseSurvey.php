...
/**
 * Base class that represents a row from the 'surveys_survey' table.
 *
 * Encuestas
 *
...
abstract class BaseSurvey extends BaseObject  implements Persistent
{
	...
	/**
	 * @var        array SurveyQuestion[] Collection to store aggregation of SurveyQuestion objects.
	 */
	...
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = SurveyPeer::ID;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = SurveyPeer::NAME;
		}
	...
	public function setIspublic($v)
	{
		...
		if ($this->ispublic !== $v || $this->isNew()) {
			$this->ispublic = $v;
			$this->modifiedColumns[] = SurveyPeer::ISPUBLIC;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->ispublic = ($row[$startcol + 2] !== null) ? (boolean) $row[$startcol + 2] : null;
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($isInsert) {
				...
				if (!$this->isColumnModified(SurveyPeer::CREATED_AT)) {
					$this->setCreatedAt(time());
				}
				if (!$this->isColumnModified(SurveyPeer::UPDATED_AT)) {
					$this->setUpdatedAt(time());
				}
			} else {
				$ret = $ret && $this->preUpdate($con);
				// timestampable behavior
				if ($this->isModified() && !$this->isColumnModified(SurveyPeer::UPDATED_AT)) {
					$this->setUpdatedAt(time());
				}
			}
			if ($ret) {
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(SurveyPeer::ID) ) {
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(SurveyPeer::ID)) $criteria->add(SurveyPeer::ID, $this->id);
		if ($this->isColumnModified(SurveyPeer::NAME)) $criteria->add(SurveyPeer::NAME, $this->name);
		if ($this->isColumnModified(SurveyPeer::ISPUBLIC)) $criteria->add(SurveyPeer::ISPUBLIC, $this->ispublic);
		if ($this->isColumnModified(SurveyPeer::STARTDATE)) $criteria->add(SurveyPeer::STARTDATE, $this->startdate);
		if ($this->isColumnModified(SurveyPeer::ENDDATE)) $criteria->add(SurveyPeer::ENDDATE, $this->enddate);
		if ($this->isColumnModified(SurveyPeer::DELETED_AT)) $criteria->add(SurveyPeer::DELETED_AT, $this->deleted_at);
		if ($this->isColumnModified(SurveyPeer::CREATED_AT)) $criteria->add(SurveyPeer::CREATED_AT, $this->created_at);
		if ($this->isColumnModified(SurveyPeer::UPDATED_AT)) $criteria->add(SurveyPeer::UPDATED_AT, $this->updated_at);
	...
	{
		$criteria = new Criteria(SurveyPeer::DATABASE_NAME);
		$criteria->add(SurveyPeer::ID, $this->id);
	...
	/**
	 * Gets an array of SurveyQuestion objects which contain a foreign key that references this object.
	 *
	 * If the $criteria is not null, it is used to always fetch the results from the database.
	 * Otherwise the results are fetched from the database the first time, then cached.
	 * Next time the same method is called without $criteria, the cached collection is returned.
	 * If this Survey is new, it will return
	 * an empty collection or the current collection; the criteria is ignored on a new object.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 * @return     PropelCollection|array SurveyQuestion[] List of SurveyQuestion objects
	 * @throws     PropelException
	 */
	public function getSurveyQuestions($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collSurveyQuestions || null !== $criteria) {
			if ($this->isNew() && null === $this->collSurveyQuestions) {
				// return empty collection
				$this->initSurveyQuestions();
			} else {
				$collSurveyQuestions = SurveyQuestionQuery::create(null, $criteria)
					->filterBySurvey($this)
					->find($con);
				if (null !== $criteria) {
	 ...
	 * @throws     PropelException
	 */
	public function countSurveyQuestions(Criteria $criteria = null, $distinct = false, PropelPDO $con = null)
	{
		if(null === $this->collSurveyQuestions || null !== $criteria) {
			if ($this->isNew() && null === $this->collSurveyQuestions) {
				return 0;
			} else {
				$query = SurveyQuestionQuery::create(null, $criteria);
				if($distinct) {
					$query->distinct();
				}
				return $query
	...
	/**
	 * Method called to associate a SurveyQuestion object to this object
	 * through the SurveyQuestion foreign key attribute.
	 *
	 * @param      SurveyQuestion $l SurveyQuestion
	 * @return     void
	 * @throws     PropelException
	 */
	public function addSurveyQuestion(SurveyQuestion $l)
	{
		if ($this->collSurveyQuestions === null) {
			$this->initSurveyQuestions();
		}
		if (!$this->collSurveyQuestions->contains($l)) { // only add it if the **same** object is not already associated
			$this->collSurveyQuestions[]= $l;
			$l->setSurvey($this);
		}
	}
