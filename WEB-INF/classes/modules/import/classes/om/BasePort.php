...
/**
 * Base class that represents a row from the 'import_port' table.
 *
 * Puerto
 *
 * @package    propel.generator.import.classes.om
 */
abstract class BasePort extends BaseObject  implements Persistent
{
  ...
  const PEER = 'PortPeer';
	 ...
	 * @return     Port The current object (for fluent API support)
	 */
	public function setId($v)
	{
		...
		if ($this->id !== $v) {
			$this->id = $v;
			$this->modifiedColumns[] = PortPeer::ID;
		}
	...
	public function setCode($v)
	{
		...
		if ($this->code !== $v) {
			$this->code = $v;
			$this->modifiedColumns[] = PortPeer::CODE;
		}
	...
	public function setName($v)
	{
		...
		if ($this->name !== $v) {
			$this->name = $v;
			$this->modifiedColumns[] = PortPeer::NAME;
		}
	...
	public function setActive($v)
	{
		...
		if ($this->active !== $v) {
			$this->active = $v;
			$this->modifiedColumns[] = PortPeer::ACTIVE;
		}
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 4; // 4 = PortPeer::NUM_COLUMNS - PortPeer::NUM_LAZY_LOAD_COLUMNS).

		} catch (Exception $e) {
			throw new PropelException("Error populating Port object", $e);
		}
	}
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = PortPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				PortPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(PortPeer::ID) ) {
					...
					$affectedRows = PortPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = PortPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = PortPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = PortPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getCode(),
			$keys[2] => $this->getName(),
	...
	public function buildCriteria()
	{
		...
		if ($this->isColumnModified(PortPeer::ID)) $criteria->add(PortPeer::ID, $this->id);
		if ($this->isColumnModified(PortPeer::CODE)) $criteria->add(PortPeer::CODE, $this->code);
		if ($this->isColumnModified(PortPeer::NAME)) $criteria->add(PortPeer::NAME, $this->name);
		if ($this->isColumnModified(PortPeer::ACTIVE)) $criteria->add(PortPeer::ACTIVE, $this->active);
	...
	{
		$criteria = new Criteria(PortPeer::DATABASE_NAME);
		$criteria->add(PortPeer::ID, $this->id);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of Port (or compatible) type.
	 ...
	 *
	 * @param      boolean $deepCopy Whether to also copy all rows that refer (by fkey) to the current row.
	 * @return     Port Clone of current object.
	 * @throws     PropelException
	 */
	 ...
	 * If this Port is new, it will return
	 * an empty collection or the current collection; the criteria is ignored on a new object.
	 *
	 * @param      Criteria $criteria optional Criteria object to narrow the query
	 * @param      PropelPDO $con optional connection object
	 * @return     PropelCollection|array Supplier[] List of Supplier objects
	 * @throws     PropelException
	 */
	public function getSuppliers($criteria = null, PropelPDO $con = null)
	{
		if(null === $this->collSuppliers || null !== $criteria) {
	...
	public function addSupplier(Supplier $l)
	{
		...
		if (!$this->collSuppliers->contains($l)) { // only add it if the **same** object is not already associated
			$this->collSuppliers[]= $l;
			$l->setPort($this);
		}
	}
	 ...
	 * @return     PropelCollection|array Supplier[] List of Supplier objects
	 */
	public function getSuppliersJoinIncoterm($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = SupplierQuery::create(null, $criteria);
