...
abstract class BaseAffiliateLevel extends BaseObject  implements Persistent
{
	 ...
	 * @return     AffiliateLevel The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->bitlevel = ($row[$startcol + 2] !== null) ? (int) $row[$startcol + 2] : null;
			$this->resetModified();
			...
			return $startcol + 3; // 3 = AffiliateLevelPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = AffiliateLevelPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	{
		if (isset($alreadyDumpedObjects['AffiliateLevel'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['AffiliateLevel'][$this->getPrimaryKey()] = true;
		...
		if ($includeForeignObjects) {
			if (null !== $this->collAffiliateUsers) {
				$result['AffiliateUsers'] = $this->collAffiliateUsers->toArray(null, true, $keyType, $includeLazyLoadColumns, $alreadyDumpedObjects);
			}
		}
		return $result;
	}
	 ...
	 * @return     void
	 */
	public function initAffiliateUsers($overrideExisting = true)
	{
		if (null !== $this->collAffiliateUsers && !$overrideExisting) {
			return;
		}
	...
	{
		return (string) $this->exportTo(AffiliateLevelPeer::DEFAULT_STRING_FORMAT);
	}
