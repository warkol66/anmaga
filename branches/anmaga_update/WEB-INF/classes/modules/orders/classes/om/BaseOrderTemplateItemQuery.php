...
abstract class BaseOrderTemplateItemQuery extends ModelCriteria
{
	 ...
	 */
	public function __construct($dbName = 'application', $modelName = 'OrderTemplateItem', $modelAlias = null)
	{
	 ...
	 * Example usage:
	 * <code>
	 * $query->filterByOrdertemplateid(1234); // WHERE orderTemplateId = 1234
	 * $query->filterByOrdertemplateid(array(12, 34)); // WHERE orderTemplateId IN (12, 34)
	 * $query->filterByOrdertemplateid(array('min' => 12)); // WHERE orderTemplateId > 12
	 * </code>
	 *
	 * @see       filterByOrderTemplate()
	 *
	 * @param     mixed $ordertemplateid The value to use as filter.
	 *              Use scalar values for equality.
	 *              Use array values for in_array() equivalent.
	 *              Use associative array('min' => $minValue, 'max' => $maxValue) for intervals.
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 ...
	 *
	 * @param     OrderTemplate|PropelCollection $orderTemplate The related object(s) to use as filter
	 * @param     string $comparison Operator to use for the column comparison, defaults to Criteria::EQUAL
	 *
	 * @return    OrderTemplateItemQuery The current query, for fluid interface
	 */
	public function filterByOrderTemplate($orderTemplate, $comparison = null)
	{
		if ($orderTemplate instanceof OrderTemplate) {
			...
			return $this
				->addUsingAlias(OrderTemplateItemPeer::ORDERTEMPLATEID, $orderTemplate->toKeyValue('PrimaryKey', 'Id'), $comparison);
		} else {
	...
	public function filterByProduct($product, $comparison = null)
	{
		if ($product instanceof Product) {
			...
			return $this
				->addUsingAlias(OrderTemplateItemPeer::PRODUCTCODE, $product->toKeyValue('PrimaryKey', 'Code'), $comparison);
		} else {
