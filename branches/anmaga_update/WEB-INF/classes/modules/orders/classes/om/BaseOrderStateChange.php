...
abstract class BaseOrderStateChange extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'OrderStateChangePeer';
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			$this->created = ($row[$startcol + 1] !== null) ? (string) $row[$startcol + 1] : null;
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = OrderStateChangePeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				OrderStateChangePeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(OrderStateChangePeer::ID) ) {
	...
	{
		$pos = OrderStateChangePeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setCreated($arr[$keys[1]]);
