...
abstract class BaseMeasureUnit extends BaseObject  implements Persistent
{
	 ...
	 * @return     MeasureUnit The current object (for fluent API support)
	 */
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 2; // 2 = MeasureUnitPeer::NUM_HYDRATE_COLUMNS.
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = MeasureUnitPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				MeasureUnitPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(MeasureUnitPeer::ID) ) {
					...
					$affectedRows = MeasureUnitPeer::doUpdate($this, $con);
				}
	...
	protected function doValidate($columns = null)
	{
		if (!$this->alreadyInValidation) {
			...
			if (($retval = MeasureUnitPeer::doValidate($this, $columns)) !== true) {
	...
	{
		$pos = MeasureUnitPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		if (isset($alreadyDumpedObjects['MeasureUnit'][$this->getPrimaryKey()])) {
			return '*RECURSION*';
		}
		$alreadyDumpedObjects['MeasureUnit'][$this->getPrimaryKey()] = true;
		$keys = MeasureUnitPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
		);
	 ...
	 * objects.
	 *
	 * @param      object $copyObj An object of MeasureUnit (or compatible) type.
	 ...
	 * @return     PropelCollection|array Product[] List of Product objects
	 */
	public function getProductsJoinUnit($criteria = null, $con = null, $join_behavior = Criteria::LEFT_JOIN)
	{
		$query = ProductQuery::create(null, $criteria);
		$query->joinWith('Unit', $join_behavior);
	...
	{
		return (string) $this->exportTo(MeasureUnitPeer::DEFAULT_STRING_FORMAT);
	}
