...
abstract class BaseInternalMail extends BaseObject  implements Persistent
{
	...
	public function setReadon($v)
	{
		...
		if ( $this->readon !== null || $dt !== null ) {
			...
			$currNorm = ($this->readon !== null && $tmpDt = new DateTime($this->readon)) ? $tmpDt->format('Y-m-d H:i:s') : null;
			$newNorm = ($dt !== null) ? $dt->format('Y-m-d H:i:s') : null;

			if ( ($currNorm !== $newNorm) // normalized values don't match 
					)
			{
				$this->readon = ($dt ? $dt->format('Y-m-d H:i:s') : null);
	...
	public function hydrate($row, $startcol = 0, $rehydrate = false)
	{
		try {
			...
			return $startcol + 13; // 13 = InternalMailPeer::NUM_COLUMNS - InternalMailPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	public function toArray($keyType = BasePeer::TYPE_PHPNAME, $includeLazyLoadColumns = true, $includeForeignObjects = false)
	{
		...
		if ($includeForeignObjects) {
			if (null !== $this->aInternalMailRelatedByReplyid) {
				$result['InternalMailRelatedByReplyid'] = $this->aInternalMailRelatedByReplyid->toArray($keyType, $includeLazyLoadColumns, true);
			}
		}
		return $result;
	}
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		$copyObj->setRecipientid($this->recipientid);
		$copyObj->setRecipienttype($this->recipienttype);
		$copyObj->setReadon($this->readon);
		$copyObj->setFromid($this->fromid);
		$copyObj->setFromtype($this->fromtype);
		$copyObj->setTo($this->to);
		$copyObj->setReplyid($this->replyid);
	 ...
	 * @return     void
	 */
	public function initInternalMailsRelatedById()
	{
	...
	public function clearAllReferences($deep = false)
	{
		if ($deep) {
			if ($this->collInternalMailsRelatedById) {
				foreach ((array) $this->collInternalMailsRelatedById as $o) {
