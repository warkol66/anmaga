...
abstract class BaseUserInfo extends BaseObject  implements Persistent
{
  ...
  const PEER = 'UserInfoPeer';
	 ...
	 * @return     UserInfo The current object (for fluent API support)
	 */
	...
	public function reload($deep = false, PropelPDO $con = null)
	{
		...
		$stmt = UserInfoPeer::doSelectStmt($this->buildPkeyCriteria(), $con);
	...
	public function save(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				...
				$this->postSave($con);
				UserInfoPeer::addInstanceToPool($this);
			} else {
				$affectedRows = 0;
			}
			$con->commit();
	...
	{
		$pos = UserInfoPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = UserInfoPeer::getFieldNames($keyType);
		$result = array(
