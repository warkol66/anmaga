...
abstract class BaseClientPurchaseOrderHistory extends BaseObject  implements Persistent
{
	 ...
	 * Peer class name
	 */
	const PEER = 'ClientPurchaseOrderHistoryPeer';
	...
	public function delete(PropelPDO $con = null)
	{
		...
		try {
			...
			if ($ret) {
				ClientPurchaseOrderHistoryQuery::create()
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(ClientPurchaseOrderHistoryPeer::ID) ) {
	...
	{
		$pos = ClientPurchaseOrderHistoryPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	public function getByPosition($pos)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				return $this->getClientpurchaseorderid();
				break;
			case 2:
	...
	{
		$keys = ClientPurchaseOrderHistoryPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getClientpurchaseorderid(),
			$keys[2] => $this->getStatus(),
			$keys[3] => $this->getCreatedat(),
		);
		if ($includeForeignObjects) {
			if (null !== $this->aClientPurchaseOrder) {
				$result['ClientPurchaseOrder'] = $this->aClientPurchaseOrder->toArray($keyType, $includeLazyLoadColumns, true);
			}
		}
		return $result;
	}
	...
	public function setByPosition($pos, $value)
	{
		switch($pos) {
			case 0:
				...
				break;
			case 1:
				$this->setClientpurchaseorderid($value);
				break;
			case 2:
	...
	public function fromArray($arr, $keyType = BasePeer::TYPE_PHPNAME)
	{
		...
		if (array_key_exists($keys[1], $arr)) $this->setClientpurchaseorderid($arr[$keys[1]]);
	...
	{
		$this->id = null;
		$this->clientpurchaseorderid = null;
		$this->status = null;
		$this->createdat = null;
		$this->alreadyInSave = false;
		$this->alreadyInValidation = false;
