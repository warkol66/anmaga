...
abstract class BaseIncoterm extends BaseObject  implements Persistent
{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->isModified()) {
				if ($this->isNew()) {
					...
					if ($criteria->keyContainsValue(IncotermPeer::ID) ) {
	...
	{
		$pos = IncotermPeer::translateFieldName($name, $type, BasePeer::TYPE_NUM);
	...
	{
		$keys = IncotermPeer::getFieldNames($keyType);
		$result = array(
			$keys[0] => $this->getId(),
			$keys[1] => $this->getName(),
	 ...
	 * If this Incoterm is new, it will return
