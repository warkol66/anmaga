...
abstract class BaseAffiliate extends BaseObject implements Persistent {
		...
		if ($this->aAffiliateInfo !== null && $this->aAffiliateInfo->getAffiliateid() !== $v) {
		  ...
		  if ($this->collOrders !== null) {
			  for ($i=0,$size=count($this->collOrders); $i < $size; $i++) {
				  $this->collOrders[$i]->setAffiliateid($v);
			  }
		  }
		  ...
		  if ($this->collOrderTemplates !== null) {
			  for ($i=0,$size=count($this->collOrderTemplates); $i < $size; $i++) {
				  $this->collOrderTemplates[$i]->setAffiliateid($v);
			  }
		  }
			  	
		  // update associated Branch		  
		  if ($this->collBranchs !== null) {
			  for ($i=0,$size=count($this->collBranchs); $i < $size; $i++) {
				  $this->collBranchs[$i]->setAffiliateid($v);
			  }
		  }
		...
		if ($this->collOrders === null) {
			if ($this->isNew()) {
			   $this->collOrders = array();
				...
				$criteria->add(OrderPeer::AFFILIATEID, $this->getId() );
		...
		if ($this->collOrderTemplates === null) {
			if ($this->isNew()) {
			   $this->collOrderTemplates = array();
				...
				$criteria->add(OrderTemplatePeer::AFFILIATEID, $this->getId() );
	...
	/**
	 * Collection to store aggregation of collBranchs	  
	 * @var array
	 */
	protected $collBranchs; 

	/**
	 * Temporary storage of collBranchs to save a possible db hit in
	 * the event objects are add to the collection, but the
	 * complete collection is never requested.
	 * @return void
	 */
	...
	/**
	 * The criteria used to select the current contents of collBranchs.
	 * @var Criteria
	 */
	private $lastBranchsCriteria = null;
	 ...
	 * @throws PropelException
	 */
	public function getBranchs($criteria = null, $con = null)
	{
		// include the Peer class
		...
		if ($this->collBranchs === null) {
			if ($this->isNew()) {
			   $this->collBranchs = array();
				...
				$criteria->add(BranchPeer::AFFILIATEID, $this->getId() );
			...
			if (!$this->isNew()) {
				...
				if (!isset($this->lastBranchsCriteria) || !$this->lastBranchsCriteria->equals($criteria)) {
					$this->collBranchs = BranchPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastBranchsCriteria = $criteria;
	 ...
	 * actually need in Affiliate.
	 */
	public function getBranchsJoinAffiliate($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'anmaga/BranchPeer.php';
		if ($criteria === null) {
		...
		if ($this->collBranchs === null) {
			if ($this->isNew()) {
			   $this->collBranchs = array();
				...
				$criteria->add(BranchPeer::AFFILIATEID, $this->getId());
				$this->collBranchs = BranchPeer::doSelectJoinAffiliate($criteria, $con);
			}
		} else {
			// the following code is to determine if a new query is
			// called for.  If the criteria is the same as the last
	...
	protected function doValidate()
	{
		if (! $this->alreadyInValidation) {
			...
			if ($this->collBranchs !== null) {
				for ($i=0,$size=count($this->collBranchs); $i < $size; $i++) {
					if (($retval = $this->collBranchs[$i]->validate()) !== true) {
						$failureMap = array_merge($failureMap, $retval);
					}
				}
			}
