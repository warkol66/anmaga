...
abstract class BaseActionLog extends BaseObject  implements Persistent {
	...
	/**
	 * Collection to store aggregation of collActionLogLabels.
	 * @var        array
	 */
	protected $collActionLogLabels;

	/**
	 * The criteria used to select the current contents of collActionLogLabels.
	 * @var        Criteria
	 */
	protected $lastActionLogLabelCriteria = null;
	...
	public function hydrate(ResultSet $rs, $startcol = 1)
	{
		try {
			...
			return $startcol + 6; // 6 = ActionLogPeer::NUM_COLUMNS - ActionLogPeer::NUM_LAZY_LOAD_COLUMNS).
	...
	protected function doSave($con)
	{
		...
		if (!$this->alreadyInSave) {
			...
			if ($this->collActionLogLabels !== null) {
				foreach($this->collActionLogLabels as $referrerFK) {
	...
	public function copyInto($copyObj, $deepCopy = false)
	{
		...
		if ($deepCopy) {
			...
			foreach($this->getActionLogLabels() as $relObj) {
				$copyObj->addActionLogLabel($relObj->copy($deepCopy));
			}
	...
	/**
	 * Temporary storage of collActionLogLabels to save a possible db hit in
	 * the event objects are add to the collection, but the
	 * complete collection is never requested.
	 * @return     void
	 */
	public function initActionLogLabels()
	{
		if ($this->collActionLogLabels === null) {
			$this->collActionLogLabels = array();
		}
	}
	 ...
	 * Otherwise if this ActionLog has previously
	 * been saved, it will retrieve related ActionLogLabels from storage.
	 * If this ActionLog is new, it will return
	 ...
	 * @throws     PropelException
	 */
	public function getActionLogLabels($criteria = null, $con = null)
	{
		// include the Peer class
		include_once 'anmaga/om/BaseActionLogLabelPeer.php';
		if ($criteria === null) {
		...
		if ($this->collActionLogLabels === null) {
			if ($this->isNew()) {
			   $this->collActionLogLabels = array();
				...
				$criteria->add(ActionLogLabelPeer::ID, $this->getId());

				ActionLogLabelPeer::addSelectColumns($criteria);
				$this->collActionLogLabels = ActionLogLabelPeer::doSelect($criteria, $con);
			}
		} else {
			...
			if (!$this->isNew()) {
				...
				ActionLogLabelPeer::addSelectColumns($criteria);
				if (!isset($this->lastActionLogLabelCriteria) || !$this->lastActionLogLabelCriteria->equals($criteria)) {
					$this->collActionLogLabels = ActionLogLabelPeer::doSelect($criteria, $con);
				}
			}
		}
		$this->lastActionLogLabelCriteria = $criteria;
		return $this->collActionLogLabels;
	}

	/**
	 * Returns the number of related ActionLogLabels.
	 *
	 * @param      Criteria $criteria
	 ...
	 * @throws     PropelException
	 */
	public function countActionLogLabels($criteria = null, $distinct = false, $con = null)
	{
		// include the Peer class
		include_once 'anmaga/om/BaseActionLogLabelPeer.php';
		if ($criteria === null) {
		...
		return ActionLogLabelPeer::doCount($criteria, $distinct, $con);
	}

	/**
	 * Method called to associate a ActionLogLabel object to this object
	 * through the ActionLogLabel foreign key attribute
	 *
	 * @param      ActionLogLabel $l ActionLogLabel
	 * @return     void
	 * @throws     PropelException
	 */
	public function addActionLogLabel(ActionLogLabel $l)
	{
		$this->collActionLogLabels[] = $l;
		$l->setActionLog($this);
	}
